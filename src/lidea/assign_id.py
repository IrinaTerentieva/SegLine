...
def create_segment_ids(centerline_gdf):
    """
    Create SegmentID column. When one centerline per footprint: SegmentID = UniqueID.
    Multiple centerlines per footprint: append suffix _2, _3, etc.
    """
    centerline_gdf = centerline_gdf.copy()
    centerline_gdf['SegmentID'] = None

    matched = centerline_gdf[centerline_gdf['UniqueID'].notna()]
    counts = matched.groupby('UniqueID').cumcount()

    for idx, cum in counts.items():
        uid = centerline_gdf.at[idx, 'UniqueID']
        if cum == 0:
            centerline_gdf.at[idx, 'SegmentID'] = uid
        else:
            centerline_gdf.at[idx, 'SegmentID'] = f"{uid}_{cum + 1}"
...
def assign_unique_ids_with_points_parallel(footprint_gdf, centerline_gdf, debug=False,
    ...
    # Issue 8: Log unmatched centerlines
    unmatched = centerline_gdf[centerline_gdf["UniqueID"].isna()]
    if len(unmatched) > 0:
        logging.warning(f"{len(unmatched)} centerlines could not be matched to any footprint:")
        for idx, row in unmatched.iterrows():
            length = row.geometry.length if row.geometry else 0
            logging.warning(f"  Unmatched centerline idx={idx}, length={length:.1f}m")

    # Issue 1: Create SegmentID
    centerline_gdf = create_segment_ids(centerline_gdf)
...
def save_footprint(footprint_gdf, footprint_updated_path):
    ...
    try:
        ...
        footprint_gdf = drop_sitetype(footprint_gdf)
...
def save_centerline(centerline_gdf, centerline_updated_path):
    ...
    try:
        ...
        centerline_gdf = drop_sitetype(centerline_gdf)
...
def warn_null_tc_trail(centerline_gdf):
    """Issue 5: Log warning for segments with null TC or Trail."""
    for col in ['TC', 'Trail']:
        if col in centerline_gdf.columns:
            null_mask = centerline_gdf[col].isna()
            if null_mask.any():
                null_ids = centerline_gdf.loc[null_mask, 'SegmentID'].tolist()
                logging.warning(f"{len(null_ids)} segments have null '{col}': {null_ids}")
...
def main(cfg: DictConfig):
    ...
    # Issue 5: Warn about null TC/Trail
    warn_null_tc_trail(centerline_gdf)
